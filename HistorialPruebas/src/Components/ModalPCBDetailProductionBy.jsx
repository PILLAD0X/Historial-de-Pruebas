...
function ModalPCBProductionBy({ show, handleClose, triggeredBy, searchCriterial, component}) {
    ...
    const { next, prev, currentData, currentPage, maxPage } = usePagination(PCBs, itemsPerPage);
    ...
    const GetPCBsByLot = async(lot) =>{
        ...
        try {
            setLoading(true);            
            const response = await axios.get(`${server}/api/PcbsManufacturedByLot?lot=${lot}&Material=${component}`);
    ...
    const GetPCBsByUID = async(uid) =>{
        ...
        try {
            ...
            const response = await axios.get(`${server}/api/PcbsManufacturedByUID?UID=${uid}&Material=${component}`);
            if (response.data.length === 0 || response === undefined || response.data[0].parent ==='') {
    ...
    if (PCBs.length > 0 & triggeredBy === 'Lot') {
      XLSX.writeFile(
        wb,
        `PCBs Manufactured by Lot ${searchCriterial} .xlsx`
      );
    } else{
        XLSX.writeFile(
            wb,
            `PCBs Manufactured by UID ${searchCriterial} .xlsx`
        );
    }
  };
