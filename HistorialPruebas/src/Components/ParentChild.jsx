...
const ParentChild = (props) => {
  ...
  const PCBTechnology = (PCB) => {
    //se utilizara para identificar si la placa fue hecha en Siplace o en panasonic y asi poder renderizar la tabla correcta y traer las pruebas.
    //console.log
  ...
  if (props.datos.parent === "" && props.datos.child !== "") {
    ...
    //When we don't have a Top and botton relation for the PCB
    return (
      <div className="divProductProperties">
        {props.datos === "No se encontro una relacion entre Parent y Child" ? (
          <h3>
            <FaIcons.FaExclamationTriangle />
            Could not find relationship between PCB and Amplifier
          </h3>
        ) : (
          <p className="lbspropiedades propiedadesT">
            <FaIcons.FaDatabase /> AmpRelation:
          </p>
        )}
        {props.datos === "No se encontro una relacion entre Parent y Child" ? (
          <h6> </h6>
