...
export const MainExecution = async (
  ...
  if (
    ...
    try {
      ...
      const parent_pcb_raltion = await GetParentChildRelation(
        serialNumber,
        setParentChild,
        setLoadingPCB
      );
      ...
      if (
        ...
        // When parent-child relationship don't exist.

        setParentChild(parent_pcb_raltion);
        if (serialNumber.length === 15) {
          // WHEN AN AMPLIFIER HASN'T A PARENT-CHILD RELATIONSHIP

          getMFGYear15LengthCodes()
            .then(async (MFGYidentifier) => {
              // we start the popup that require mfgyear
              if (MFGYidentifier) {
                var responseTestHistoryF = await GetTestHistory(
                  serialNumber,
                  MFGYidentifier,
                  "GM15Serial",
                ...
                if (responseTestHistoryF === "Could not found test history") {
                  // if we couldn't get a test history we going to check if we can get GMserial realtio
          ...
          //When the serial number  haven't parent -child relation and is longer that 15 characters

          if (serialNumber.length === 23) {
            //if the serial number is in format 23 start the search process.
        ...
        if (
          ...
          //IF the parent serial lenght is equal to 15 or 23 we do this.

          GetTestHistory(
            parent_pcb_raltion.parent,
            parent_pcb_raltion.mfg_year,
            "FA",
            { setPruebasCodigoNoIdentif, setPruebasFA, setPruebasPCB },
            setLoadingPCB,
        ...
        } else if (parent_pcb_raltion.parent.length > 23) {
          //If the
          let detail70serial = JSON.parse(
            ParseSerialNumberTo23(parent_pcb_raltion.parent)
          );
          setdetalle70Barcode(detail70serial);
          const mfgYear = detail70serial.code23.substr(1, 1); // serialNumber.substr(1,1)
          GetTestHistory(
    ...
    Swal1(
      "ATENCION!",
      "El numero de serie ingresado no cumple con la longitud estandar, la logitud estandar permitida para una serie es de: 15, 23, 36, 43, 45, 55 ó 70 caracteres.",
      "error"
    );
  }
};
